...
# Usando o serviço do S3 para armezenar o modelo

resource "aws_s3_bucket" "dsa_bucket_flask" {
  bucket = "dsa-495599753255-bucket" # Nome do bucket deve ser único em toda a AWS e o código do bucket é o id da conta aws
  tags = {
    Name        = "DSA Bucket"
    Environment = "Lab4"
  }
  ...
  provisioner "local-exec" {
    when    = destroy
    command = "aws s3 rm s3://dsa-495599753255-bucket --recursive"
  }
}

# Usando o serviço do EC2 para criar uma instância
resource "aws_instance" "dsa_ml_api" {
  ...
  user_data = <<-EOF
                ...
                sudo aws s3 sync s3://dsa-495599753255-bucket /dsa_ml_app 
                cd /dsa_ml_app
...
# Criando o bucket do S3 para armazenar o modelo e a role
